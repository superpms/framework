...
class ValidateHttpRequestMiddleware extends Middleware
{
    ...
    public function handle():void{
        foreach (['method','validate','login','terminal','token'] as $paramKey){
            $item = $this->class->getProperty($paramKey);
            $this->config[$paramKey] = $item->getDefaultValue();
        }

        $this->method();
        $vToken = $this->config['token'];
        if($vToken){
            $this->token();
        }
        $this->auth();
        $this->params($this->config['validate'] ?? [],$this->request->params());
    }
    ...
    private function params(array $config,array $paramsDta): void{
        $realData = [];
    ...
    public function callback(ContainerInterface &$server): void{
        $server->put('pms\inject\SafeParams',new SafeParams($this->realParams));
    }
}