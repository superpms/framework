...
class  Setup implements LifecycleInterface
{
    ...
    {
        $fnList = BootOptions::get_php()?->validate?->function ?? [];
        foreach ($fnList as $item) {
            if (!str_starts_with($item, '#') && !function_exists($item)) {
                throw new \Exception("启动检测-PHP函数 {$item} 无法使用");
            }
        }
    }

    public static function validatePhpExtension(): void{
        $extList = BootOptions::get_php()?->validate?->extension ?? [];
        foreach ($extList as $item) {
            if (!str_starts_with($item, '#') && !extension_loaded($item)) {
                throw new \Exception("启动检测-PHP扩展 {$item} 尚未安装");
            }
        }
    }
